#include "cam/optical_detection.h"
...
void image_callback(const sensor_msgs::ImageConstPtr &img)
{
    ...
    goodFeaturesToTrack(gray, kps_3, MAX_COUNT, 0.01, 10, Mat(), 3, 3, 0, 0.04);
    cornerSubPix(gray, kps_3, subPixWinSize, Size(-1,-1), termcrit);
    ...
    if(has_got_first_frame)
    {
        ...
        int img_dis_count = 0;
        for (int j = 0; j < kps_2.size(); ++j)
        {
          float dx = kps_1[j].x - kps_2[j].x;
          float dy = kps_1[j].y - kps_2[j].y;
        ...
        {
          Mat R, t;
          pose_estimation_2d2d(kps_1, kps_2, R, t);

          std::vector<Point3d> points;
          triangulation( kps_1, kps_2, R, t, points );
          std::cout<<kps_1.size()<<"--"<<kps_2.size()<<std::endl;

          cloud.points.resize(points.size());

          myClusterAnalysis.clear();
          myClusterAnalysis.Init(points,4,3);
          ...
          for(int i=0; i<points.size(); i++)
          {
            ...
            if (cluster_id != -1)
            {
              //circle(cur_frame, kps_2[i], 3, Scalar(0, 255-cluster_id*step, cluster_id*step), -1, 8);
            ...
            point_cluster.push_back(cluster_id);
        ...
        else
        {
          if(has_got_cluster)
          {
            ...
            for(int i=0; i<point_cluster.size(); i++)
            {
                ...
                int cluster_id = point_cluster[i];
                if(cluster_id != -1)
                {
                            ...
                            Mat& R, Mat& t )
{
    ...
    fundamental_matrix = findFundamentalMat ( pts_1, pts_2, CV_FM_8POINT );
    ...
    essential_matrix = findEssentialMat ( pts_1, pts_2, focal_length, principal_point );
    //cout<<"essential_matrix is "<<endl<<essential_matrix<<endl;

    //-- 计算单应矩阵
    Mat homography_matrix;
    homography_matrix = findHomography ( pts_1, pts_2, RANSAC, 3 );
    //cout<<"homography_matrix is "<<endl<<homography_matrix<<endl;

    //-- 从本质矩阵中恢复旋转和平移信息.
    recoverPose ( essential_matrix, pts_1, pts_2, R, t, focal_length, principal_point );
    cout<<"R is "<<endl<<R<<endl;
    cout<<"t is "<<endl<<t<<endl;

}

void triangulation (const vector< Point2f >& pts_1,
    ...
    vector< Point3d >& points )
{
    ...
    {
        // 将像素坐标转换至相机坐标
        points_1.push_back ( pixel2cam( pts_1[i], K) );
        points_2.push_back ( pixel2cam( pts_2[i], K) );
    }
