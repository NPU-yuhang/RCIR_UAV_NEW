#include "mymission/avoid_obstacle_v2.h"

int main(int argc, char** argv)
{
  ros::init(argc, argv, "avoid_obstacle_v2");
  ros::NodeHandle nh;
  ...
  goal_point = nh.advertise<geometry_msgs::Point>("/local_planner/goal", 100);
  obsta_rects = nh.advertise<cam::rects>("/local_planner/obstacle", 100);
  ...
  ros::Subscriber pathsub     = nh.subscribe("/local_planner/local_path_2", 10, &path_callback);
...
void gps_callback(const sensor_msgs::NavSatFix::ConstPtr& msg)
{
  ...
  if(elapsed_time > ros::Duration(0.02))
  {
    ...
    if(square_mission.state == 2)
    {
      ...
      if(!has_got_path && !has_send_goal)
      {
        if(data_is_available())
        {
          trans_yaw = square_mission.toEulerAngle(square_mission.current_atti).z - C_PI/2;
          std::vector<float> p_body = trans_2D(vision_task_param.goal_x - current_local_pos.x, vision_task_param.goal_y - current_local_pos.y, -trans_yaw);
          goal.x = p_body[0];
          goal.y = p_body[1];
          std::cout<<"goal: "<<goal.x<<"---"<<goal.y<<std::endl;
          goal.z = vision_task_param.goal_z - current_local_pos.z;
          goal_point.publish(goal);
          has_send_goal = true;
        }
        square_mission.state = 2;
      }
      else if(has_got_path)
    ...
    if (square_mission.state == 3)
    {
      ...
      float t_path = sqrt((goal.x)*(goal.x) + (goal.y)*(goal.y))/0.7;
      std::cout<<"---path time: "<<t_path<<std::endl;
      if(visionok_time <= t_path && has_got_path)
      {
        int path_i = (visionok_time/t_path)*rrt_path.poses.size();
        std::cout<<"path_i: "<<path_i<<"--path size: "<<rrt_path.poses.size()<<std::endl;
        float x = rrt_path.poses[path_i].pose.position.x;
        float y = rrt_path.poses[path_i].pose.position.y;
        std::vector<float> p_ground = trans_2D((double)x, (double)y, trans_yaw);
        square_mission.setTarget(p_ground[0], p_ground[1], vision_height, start_yaw);
        square_mission.state = 3;
        square_mission.step();
        std::cout<<"x: "<<current_local_pos.x<<"---y: "<<current_local_pos.y<<std::endl;
      }
      else if (visionok_time <= 15)
      {
    ...
    {
      std::cout<<"start mode 4"<<std::endl;
...
//               max_error_1y<<std::endl;
    if((std::abs(max_error_0x) < 0.5) && (std::abs(max_error_0y) < 0.5) &&
        ...
        error_is_ok = true;
    std::cout<<"error is ok: "<<error_is_ok<<std::endl;
    return error_is_ok;
}
...
std::vector<float> trans_2D(double x, double y, double yaw)
{
  ...
  p_b.push_back(x*std::cos(yaw) + y*std::sin(yaw));
  p_b.push_back(x*std::sin(yaw) + y*std::cos(yaw));
  return p_b;
}
