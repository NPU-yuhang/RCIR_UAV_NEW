...
void artifical_potencial_field::path_planning()
{
  ...
  for(j = 0; j<iteration; j++)
  {
    ...
    path_out.open("/media/nvidia/SD1/catkin_m100/src/Onboard-SDK-ROS/dji_sdk_demo/output/path.txt", std::ios::out | std::ios::app);
    path_s = std::to_string(goi.x) + " " + std::to_string(goi.y) + "\n";
    path_out << path_s;
    path_out.close();
    ...
    if(((std::abs(current_x.x) - std::abs(Xsum[0].x)) > 0)&&((std::abs(current_x.y) - std::abs(Xsum[0].y)) > 0))
    {
      count = j;
      break;
    }
...
void artifical_potencial_field::SetObstacle(std::vector<cv::Point2f> obs)
{
  ...
  {
    obsta_out.open("/media/nvidia/SD1/catkin_m100/src/Onboard-SDK-ROS/dji_sdk_demo/output/obsta.txt", std::ios::out | std::ios::app);
    obsta_s = std::to_string(obs[i].x) + " " + std::to_string(obs[i].y) + "\n";
    obsta_out << obsta_s;
    obsta_out.close();
  }
}
